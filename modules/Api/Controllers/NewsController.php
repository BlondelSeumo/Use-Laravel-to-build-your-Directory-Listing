...
class NewsController extends Controller
{
    ...
    public function category(Request $request){
        $model_News = NewsCategory::query()->select("bc_news_category.*");
        $model_News->where("bc_news_category.status", "publish");
        if (!empty($search = $request->query("s"))) {
            $model_News->where(function($query) use ($search) {
                $query->where('bc_news_category.name', 'LIKE', '%' . $search . '%');
            });

            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app_get_locale() ){
                $model_News->leftJoin('bc_news_category_translations', function ($join) use ($search) {
                    $join->on('bc_news_category.id', '=', 'bc_news_category_translations.origin_id');
                });
                $model_News->orWhere(function($query) use ($search) {
                    $query->where('bc_news_category_translations.title', 'LIKE', '%' . $search . '%');
                });
            }
        }
