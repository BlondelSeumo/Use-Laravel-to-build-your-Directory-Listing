...
class ListNews extends BaseBlock
{
    ...
    public function query($model){
        $model_news = News::select("bc_news.*")->with(['translations']);
        if(empty($model['order'])) $model['order'] = "id";
        if(empty($model['order_by'])) $model['order_by'] = "desc";
        if(empty($model['limit'])) $model['limit'] = 5;
        if (!empty($model['category_id'])) {
            ...
            if(!empty($list_cat)){
                ...
                foreach ($list_cat as $cat){
                    $where_left_right[] = " ( bc_news_category._lft >= ? AND bc_news_category._rgt <= ? ) ";
                    $params[] = $cat->_lft;
                    $params[] = $cat->_rgt;
                }
                $sql_where_join = " ( ".implode("OR" , $where_left_right)." )  ";
                $model_news
                    ->join('bc_news_category', function ($join) use($sql_where_join,$params) {
                        $join->on('bc_news_category.id', '=', 'bc_news.cat_id')
                            ->WhereRaw($sql_where_join,$params);
                    });
            }
        }

        $model_news->orderBy("bc_news.".$model['order'], $model['order_by']);
        $model_news->where("bc_news.status", "publish");
        $model_news->groupBy("bc_news.id");
        return $model_news->with(['getCategory'])->limit($model['limit'])->get();
    }
}
