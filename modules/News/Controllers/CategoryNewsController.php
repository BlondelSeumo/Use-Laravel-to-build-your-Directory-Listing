...
class CategoryNewsController extends FrontendController
{
    ...
    public function index(Request $request, $slug)
    {
        ...
        $listNews = News::query();
        $listNews->select("bc_news.*")
                ->join('bc_news_category', function ($join) use($cat) {
                    $join->on('bc_news_category.id', '=', 'bc_news.cat_id')
                         ->where('bc_news_category._lft', '>=', $cat->_lft)
                         ->where('bc_news_category._rgt', '<=', $cat->_rgt);
                })
                ->where("bc_news.status", "publish")
                ->groupBy('bc_news.id');
