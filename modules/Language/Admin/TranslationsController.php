...
class TranslationsController extends AdminController
{
    ...
    public function detail(Request $request, $id)
    {
        ...
        $query->where('bc_translations.locale', 'raw');

        $query->leftJoin('bc_translations as t', function ($join) use ($lang) {

            $join->on('t.parent_id', '=', 'bc_translations.id');
        ...
        if( $request->search_by == "translated_text"){
            ...
            if ($request->s) {
                $query->where('bc_translations.string', 'like', '%' . $request->s . '%');
            }
        }

        $origins = $query->orderBy('bc_translations.string', 'asc')->paginate(30);
    ...
    public function build($id)
    {
        ...
        $query = Translation::select([
            'bc_translations.*',
            't.string as origin'
        ])->where('bc_translations.locale', $lang->locale)->whereRaw("IFNULL(bc_translations.string,'') != '' ");
        $query->join('bc_translations as t', function ($join) use ($lang) {

            $join->on('t.id', '=', 'bc_translations.parent_id');
    ...
    public function genDefault(){
        ...
        $query = Translation::select([
            'bc_translations.*',
        ])->where('bc_translations.locale', 'raw');
        $json = [];
        $rows = $query->get();
        if (!empty($rows)) {
            foreach ($rows as $row) {
