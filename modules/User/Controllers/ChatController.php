...
class ChatController extends FrontendController
{
    ...
    {
        $tableNameMessage = (new ChMessage())->getTable();
        $user_id = intval($request->query('user_id'));
        $tmpUser = $user_id ? User::find($user_id) : false;
        // get all users that received/sent message from/to [Auth user]
        $query = ChMessage::query()->join('users',  function ($join) use ($tableNameMessage) {
            $join->on($tableNameMessage.'.from_id', '=', 'users.id')
                ->orOn($tableNameMessage.'.to_id', '=', 'users.id');
        })
            ->where($tableNameMessage.'.from_id', Auth::user()->id)
            ->orWhere($tableNameMessage.'.to_id', Auth::user()->id);

        if($user_id){
            $query->orderByRaw('CASE when users.id = ? then 1 else 2 end',[$user_id]);
        }
        $query->orderBy($tableNameMessage.'.created_at', 'desc');
        $users = $query->get()
            ->unique('id');
